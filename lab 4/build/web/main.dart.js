...
define('zapp_user_main', ['dart_sdk', 'flutter_sdk'], (function load__zapp_user_main(dart_sdk, flutter_sdk) {
  ...
  dart.defineLazy(CT, {
    ...
    get C4() {
      return C[4] = dart.constSet(core.String, ["mailto", "tel", "sms"]);
    },
    get C6() {
    ...
    get C5() {
      return C[5] = dart.constList([C[6] || CT.C6, C[7] || CT.C7, C[8] || CT.C8, C[9] || CT.C9], types.LaunchMode);
    },
    get C10() {
  ...
  main.MyApp = class MyApp extends framework.StatelessWidget {
    build(context) {
      return new app.MaterialApp.new({home: new main.HomeScreen.new()});
    }
  ...
  var _likePhoto = dart.privateName(main, "_likePhoto");
  var _copyPhoneNumber = dart.privateName(main, "_copyPhoneNumber");
  var _openGoogleMaps = dart.privateName(main, "_openGoogleMaps");
  var _shareLocation = dart.privateName(main, "_shareLocation");
  ...
  main._HomeScreenState = class _HomeScreenState extends framework.State$(main.HomeScreen) {
    ...
    [_copyPhoneNumber]() {
      return async.async(dart.void, (function* _copyPhoneNumber() {
        yield clipboard.Clipboard.setData(new clipboard.ClipboardData.new({text: "+7 (123) 456-78-90"}));
        scaffold.ScaffoldMessenger.of(this.context).showSnackBar(new snack_bar.SnackBar.new({content: new text.Text.new("Номер телефона скопирован: " + "+7 (123) 456-78-90")}));
      }).bind(this));
    }
    [_openGoogleMaps]() {
      return async.async(dart.void, function* _openGoogleMaps() {
        let address = "Улица Калинина 13, корпус 20, ул. Калинина 13, 13к20, Краснодар, Краснодарский край, 350044";
        let url = "https://www.google.com/maps/search/?api=1&query=" + address;
        if (yield legacy_api.canLaunch(url)) {
          yield legacy_api.launch(url);
        } else {
          dart.throw("Не удалось открыть Google Карты: " + url);
        }
      });
    }
    [_shareLocation]() {
      return async.async(dart.void, (function* _shareLocation() {
        let address = "Улица Калинина 13, корпус 20, ул. Калинина 13, 13к20, Краснодар, Краснодарский край, 350044";
        let url = "https://www.google.com/maps/search/?api=1&query=" + address;
        yield clipboard.Clipboard.setData(new clipboard.ClipboardData.new({text: url}));
        scaffold.ScaffoldMessenger.of(this.context).showSnackBar(new snack_bar.SnackBar.new({content: new text.Text.new("Ссылка на карту скопирована: " + url)}));
      }).bind(this));
    }
    build(context) {
      return new scaffold.Scaffold.new({appBar: new app_bar.AppBar.new({title: new text.Text.new("Общежития КубГАУ"), backgroundColor: colors.Colors.green}), body: new single_child_scroll_view.SingleChildScrollView.new({child: new basic.Column.new({children: T.JSArrayOfWidget().of([new image.Image.network("https://i.ibb.co/Xr7mScZg/12.png", {width: 1 / 0, height: 400}), new basic.Padding.new({padding: C[1] || CT.C1, child: new basic.Column.new({crossAxisAlignment: flex.CrossAxisAlignment.start, children: T.JSArrayOfWidget().of([new basic.Row.new({mainAxisSize: flex.MainAxisSize.min, children: T.JSArrayOfWidget().of([new text.Text.new("Общежитие №20", {style: new text_style.TextStyle.new({fontSize: 24, fontWeight: ui.FontWeight.bold})}), new basic.SizedBox.new({width: 400}), new icon_button.IconButton.new({icon: new icon.Icon.new(icons.Icons.favorite, {color: this.isLiked ? colors.Colors.red : colors.Colors.grey}), onPressed: dart.bind(this, _likePhoto)}), new text.Text.new(dart.str(this.likeCount), {style: new text_style.TextStyle.new({fontSize: 16, color: colors.Colors.black})})])}), new basic.SizedBox.new({height: 8}), new text.Text.new("Краснодар, ул. Калинина, 13", {style: new text_style.TextStyle.new({fontSize: 16})}), new basic.SizedBox.new({height: 16}), new basic.Row.new({mainAxisAlignment: flex.MainAxisAlignment.spaceAround, children: T.JSArrayOfWidget().of([new basic.Column.new({children: T.JSArrayOfWidget().of([new icon_button.IconButton.new({icon: new icon.Icon.new(icons.Icons.call, {color: colors.Colors.green}), onPressed: dart.bind(this, _copyPhoneNumber)}), new text.Text.new("Позвонить", {style: new text_style.TextStyle.new({color: colors.Colors.green})})])}), new basic.Column.new({children: T.JSArrayOfWidget().of([new icon_button.IconButton.new({icon: new icon.Icon.new(icons.Icons.directions, {color: colors.Colors.green}), onPressed: dart.bind(this, _openGoogleMaps)}), new text.Text.new("Маршрут", {style: new text_style.TextStyle.new({color: colors.Colors.green})})])}), new basic.Column.new({children: T.JSArrayOfWidget().of([new icon_button.IconButton.new({icon: new icon.Icon.new(icons.Icons.share, {color: colors.Colors.green}), onPressed: dart.bind(this, _shareLocation)}), new text.Text.new("Поделиться", {style: new text_style.TextStyle.new({color: colors.Colors.green})})])})])}), new basic.SizedBox.new({height: 16}), new text.Text.new("Студенческий городок или так называемый кампус Кубанского ГАУ состоит из двадцати общежитий, в которых проживает более 8000 студентов, что составляет 96% от всех нуждающихся. Студенты первого курса обеспечены местами в общежитии полностью. В соответствии с Положением о студенческих общежитиях университета, при поселении между администрацией и студентами заключается договор найма жилого помещения. Воспитательная работа в общежитиях направлена на улучшение быта, соблюдение правил внутреннего распорядка, отсутствия асоциальных явлений в молодежной среде. Условия проживания в общежитиях университетского кампуса полностью отвечают санитарным нормам и требованиям: наличие оборудованных кухонь, душевых комнат, прачечных, читальных залов, комнат самоподготовки, помещений для заседаний студенческих советов и наглядной агитации. С целью улучшения условий быта студентов активно работает система студенческого самоуправления - студенческие советы организуют всю работу по самообслуживанию.", {style: new text_style.TextStyle.new({fontSize: 16})}), new basic.SizedBox.new({height: 16})])})})])})})});
    }
    static ['_#new#tearOff']() {
      return new main._HomeScreenState.new();
    }
  ...
  dart.setMethodSignature(main._HomeScreenState, () => ({
    ...
    [_likePhoto]: dart.fnType(dart.void, []),
    [_copyPhoneNumber]: dart.fnType(dart.void, []),
    [_openGoogleMaps]: dart.fnType(dart.void, []),
    [_shareLocation]: dart.fnType(dart.void, []),
    build: dart.fnType(framework.Widget, [framework.BuildContext])
  }));
  ...
  dart.setFieldSignature(main._HomeScreenState, () => ({
    ...
    likeCount: dart.fieldType(core.int),
    isLiked: dart.fieldType(core.bool)
  }));
  ...
  url_launcher_uri.launchUrl = function launchUrl(url, opts) {
    ...
    let webViewConfiguration = opts && 'webViewConfiguration' in opts ? opts.webViewConfiguration : C[10] || CT.C10;
    let webOnlyWindowName = opts && 'webOnlyWindowName' in opts ? opts.webOnlyWindowName : null;
